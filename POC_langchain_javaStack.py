...
class YAMLGenerator:
    def __call__(self, tech_stack: str) -> str:
        prompt = f"""
        Generate a valid azure-pipelines.yml for a {tech_stack} project that:
        - Runs on push to main
        - Installs dependencies
        - Runs tests

        Use Maven for Java.
        Output only valid YAML code, no markdown or explanations.
        Make sure the YAML includes:
          - 'trigger' block
          - 'pool' block
          - only one Maven@3 task with 'goals: clean install'
...
if __name__ == "__main__":
    os.environ["AZURE_OPENAI_API_KEY"] = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
    os.environ["AZURE_OPENAI_ENDPOINT"] = "https://poc-openai-1.openai.azure.com"
    os.environ["AZURE_OPENAI_DEPLOYMENT_NAME"] = "gpt-4o-mini"
    os.environ["GITHUB_TOKEN"] = "ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
    ...
    user_input = "Generate a pipeline for Java with Maven and commit to GitHub repo https://github.com/jstar-dev/Sample on main branch"
